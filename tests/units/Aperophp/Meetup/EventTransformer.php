...
EOF;
        ...
        $this
            ->if($transformer = new \Aperophp\Meetup\EventTransformer($this->getBaseTestCities()))
            ->then
                ->array($transformer->transform($this->getBaseTestEvent(array())))
    ...
    public function testEventNonApero()
    {
        $this
            ->if($transformer = new \Aperophp\Meetup\EventTransformer($this->getBaseTestCities()))
            ->then
                ->variable($transformer->transform($this->getBaseTestEvent(array(
                    'name' => 'Session de lightning talks',
                ))))
                ->isNull()
        ;
    }

    protected function getBaseTestCities()
    {
        return array(
            125 => 'Luxembourg',
            126 => 'Lyon',
    ...
    protected function getBaseTestEvent(array $customData)
    {
